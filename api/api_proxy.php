...
file_put_contents(__DIR__ . '/error_log', "[" . date('Y-m-d H:i:s') . "] tournamentData: " . var_export($tournamentData, true) . "\n", FILE_APPEND);
file_put_contents(__DIR__ . '/error_log', "[" . date('Y-m-d H:i:s') . "] rosterData: " . var_export($rosterData, true) . "\n", FILE_APPEND);

// Validar datos antes de procesar
if (!$tournamentData || !$rosterData || !isset($tournamentData['event']['scopes'][0]['aggregates'])) {
    header('Content-Type: application/json');
    echo json_encode([
        'error' => 'No se pudo obtener datos válidos de la API',
...
foreach ($rosterData as $entry) {
    if (!isset($entry['member'])) continue;
    $member = $entry['member'];
